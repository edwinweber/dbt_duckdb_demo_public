...
## Architecture, Implementation, and Working Code in One Volume

**Version 1.1 — March 2026**

*A complete handbook for building a budget-conscious, metadata-driven data platform with open-source tooling, illustrated and implemented through the Danish Democracy Data project in this repository.*
...
This handbook is written to close that gap.

It combines a general platform architecture with a real implementation. The architectural patterns are not hypothetical. They are exercised in code in this repository: extraction with dlt, orchestration with Dagster, transformation with dbt and DuckDB, export to Delta Lake on Microsoft Fabric OneLake, and downstream consumption in Power BI.

The result is intended to be a complete work: a technical book that explains why the platform is shaped the way it is, how each layer works, how the repository is structured, how the runtime behaves, and how to operate or extend the platform without turning it into an opaque pile of scripts.
...
The intended reading experience is cumulative. Early chapters establish the vocabulary and design boundaries. Middle chapters explain the implementation. Later chapters turn the repository into a teachable reference system and operational handbook.

## Intended Audience

This handbook is written for advanced practitioners rather than beginners. It assumes the reader is already comfortable with:
...
It does not assume prior expertise in dlt, Dagster, dbt, DuckDB, or Microsoft Fabric. Those tools are explained in context through the actual implementation.
...
The modern data platform described in this handbook is built for a specific operating reality: a small team, modest budget, a need for real governance and lineage, and no appetite for paying enterprise-platform prices before the business case exists.
...
This dual role is the central premise of the book. Theory is valuable only when it predicts implementation shape. Implementation is valuable only when it can be generalized into architecture.

## Chapter 2: Design Principles

Several design principles recur throughout the repository and are worth stating explicitly.
...
Not everything is generated. The codebase deliberately mixes generated SQL with hand-written SQL. `individual_votes` is manual because it expresses business logic, not mechanical duplication. This is a healthy asymmetry, not an inconsistency.
...
The repository is organized around platform responsibilities rather than arbitrary technical layers. That sounds obvious, but many data repositories fail precisely here: they become a miscellaneous pile of notebooks, scripts, and SQL fragments. This project does not. There is a clear boundary between ingestion, transformation, orchestration, and support utilities.
...
This is the pattern of a serious production project: dependencies reflect operational knowledge, not just feature wish lists.
...
That is a good example of small design work paying large operational dividends.
...
This is a practical pattern worth generalizing.
...
This gives the platform lineage, observability, and recomposability. Those are the attributes that distinguish a modern data platform from a scheduled script folder.
...
The Bronze models are generated, not hand-authored, because the logic is repetitive and semantically shallow. This is exactly where generation belongs.
...
That pattern is valuable because it centralizes structural decisions into macros while keeping the repository contents explicit. The generated SQL files still exist and can still be reviewed, but the logic is not duplicated 18 times by hand.
...
The core Silver macro implements hash-based change detection, versioning, and delete handling. It is the most important transformation primitive in the repository.
...
This is a mature pattern for source systems where:
...
The distinction between Silver history tables and `_cv` convenience views is important. The platform preserves history without forcing every consumer to become an expert in historized joins.
...
- Semantic dimensions such as `actor.sql` should remain readable SQL.
- Fact tables such as `individual_votes.sql` should absolutely be written by humans, because the business grain and join logic are part of the model’s meaning.

This project gets that boundary right.
...
This is a platform engineering lesson worth repeating: configuration becomes powerful when it is both central and executable.

## Chapter 14: Code Generation over Repetition

The file `ddd_python/ddd_dbt/generate_dbt_models.py` contains the project’s most explicit stance on maintainability: if the SQL pattern is mechanical, generate it.
...
That restraint is one of the reasons the codebase remains legible.

## Chapter 15: Asset Factories and Dependency-Driven Execution

The Dagster extraction and export layers both use factory functions to define assets. This is a strong fit because the assets differ by parameterization, not by structure.
...
This is precisely the kind of structure that allows a small team to operate a non-trivial platform without accumulating orchestration entropy.
...
The project also normalizes Danish entity names to ASCII-safe path and model names, converting characters such as `ø`, `æ`, and `å`. This is not cosmetic. It prevents filesystem and engine friction while preserving source meaning at the semantic layer.

## Chapter 17: Testing Strategy

The tests in this repository are not ornamental. They are aimed at structural correctness.
...
That is exactly the sort of test a metadata-driven platform should have. It checks the contract between configuration and code generation rather than merely checking that a function returned a string.
...
In practice, the most important cost categories are:
...
That said, low cost comes with responsibility. You are choosing composable tools over managed abstraction. That is often the right bargain, but it is still a bargain.
...
At that size, BI licensing can easily dominate infrastructure cost. This is a useful corrective for students who assume compute is always the largest line item. In many modern analytics stacks, the visualization and sharing layer determines the commercial shape of the platform more than the transformation layer does.
...
A strong handbook must not only explain the current system. It must explain how to change it safely.
...
The deeper architectural lesson is this: extension is easy when the platform’s core abstractions are stable. In this repository, those abstractions are layer membership, factory-defined assets, macro-driven model families, and explicit export boundaries.

## Chapter 22: What This Architecture Gets Right

This design makes several good decisions that are easy to underestimate.
...
In many platform discussions, storage is treated as a passive implementation detail. That is a mistake. Storage is not passive. It determines persistence semantics, cost posture, interoperability options, recovery strategy, and the practical meaning of “done” in a pipeline run.
...
This is one of the strongest architectural ideas in the project. It keeps the runtime lightweight without making the data estate fragile.
...
For advanced students, this is a useful pattern to study because it demonstrates that platform quality often depends on what is made explicit.
...
This is exactly the right architecture for a platform that wants both low runtime complexity and durable downstream consumption. A local analytical engine becomes operationally viable when it is paired with a durable, interoperable storage layer.
...
This is not bloat. It is the minimum integration surface needed for a lightweight lakehouse pipeline.
...
This is an important lesson for advanced practitioners. Performance is not about indiscriminately adding concurrency. It is about matching concurrency to engine semantics.

### Strategic implication

The most important thing DuckDB enables here is not just speed. It enables a different kind of platform design: one where serious data engineering can happen without a permanent warehouse control plane. That architectural possibility is what makes DuckDB worth teaching in this context.
...
The compiler analogy is useful because it corrects a common misunderstanding. dbt is not simply a folder of SQL files that happen to run in order. It is a graph construction and execution framework.
...
The design lines up storage choices with semantic responsibility. That is exactly what advanced students should learn to do.
...
That is why a model like `individual_votes.sql` is executed by the Gold job even though it is not part of the Gold generation list. It exists in the dbt graph, so the manifest includes it. This distinction is subtle, and understanding it is a sign of real platform fluency.
...
That is the right mental model for advanced data engineering.

## Chapter 27: Delete Handling and Historical Correctness

Delete handling is one of the places where data platforms either mature or fail. Inserts and updates are comparatively easy. Deletes are where temporal correctness, operational assumptions, and downstream trust all meet.

The Silver macro in this project addresses deletes explicitly rather than treating them as an inconvenient corner case. That alone puts the implementation ahead of many production systems.
...
Delete handling is where students move from “I can write transformations” to “I understand data systems.” A serious course should spend time here, because temporal correctness is one of the hardest things to retrofit after the fact.
...
### Why this is a strong serving pattern
...
That is the right design for most analytical platforms operating at this scale.
...
That is exactly the kind of architectural distinction advanced students should learn to make.

## Chapter 29: Observability, Logging, and Operational Feedback Loops

Pipelines that only write business data are incomplete. Mature platforms also write operational data about their own behavior.
...
This is not elaborate, but it is highly effective. Small teams often cannot justify a large observability stack. Writing structured run summaries into the same lakehouse estate is a practical alternative.
...
That choice deserves attention. Observability is important, but it should not become a new single point of failure for the platform.
...
That is the beginning of platform observability as an engineering discipline rather than a checklist item.
...
That distinction is useful and should be explicit in a course text.
...
Advanced students should learn to separate repository scope from deployment scope. A project can fully implement its pipeline logic without hardcoding every detail of the production runtime shell into the same codebase.
...
Students should inspect the repository and explain why `individual_votes` is not code-generated and why it is not part of the Gold generation list.

The correct explanation is that `individual_votes` is a semantic fact model, not a repetitive infrastructure model. It appears in the dbt manifest and therefore in the Gold execution graph, but it is not part of the mechanical generation flow that creates `_cv` companions.

This is one of the most valuable lessons in the repository: not everything should be generated just because some things are.
...
One of the central lessons for advanced students is that the Gold layer should not be confused with reports or dashboards. Gold is a semantic data product. It is the layer where the data model becomes intentionally consumable, but it should remain independent of any single report canvas.
...
Power BI performs best in a well-governed platform when it is connecting to curated entities whose grain, keys, and update semantics are explicit. That is exactly what the Gold layer is designed to provide.
...
The broader platform principle is this: BI tools should consume exported and governed contracts, not execution internals.
...
This is one of the most important ideas in the whole book. It protects downstream consumers from the instability of the transformation runtime.
...
An advanced course should force students to think about this explicitly. Architecture that ignores licensing is not complete architecture.
...
The key architectural rule remains stable regardless of which serving mode is chosen: Power BI should read a curated and exported Gold contract rather than coupling itself to the transformation runtime.

### DEV and PROD semantic isolation

The original handbook emphasizes DEV and PROD workspace separation, and that pattern remains valuable here. Even when the repository core is identical across environments, the semantic consumption layer should preserve a meaningful separation between experimental and published artifacts.

Students should learn that promotion is not just a code event. It is also a semantic-contract event.
...
Students should distinguish carefully between two scopes.
...
Advanced students should treat dependency declarations as architecture clues.
...
This repository is already structured to support that style of discipline. A course should make that explicit rather than treating CI/CD as an unrelated DevOps concern.

### GitHub Actions as DEV runtime

One of the strongest ideas in the original handbook is that the DEV environment can run directly on the GitHub Actions runner rather than through a more elaborate intermediate environment. That is a pragmatic choice:
...
That is exactly the sort of portability this repository encourages.

### Interactive development still matters

Even with a strong CI/CD posture, advanced data work still benefits from an interactive environment. That is why Codespaces or local VS Code setups remain relevant. The right operational pattern is not to choose between interactive development and automation, but to make them converge on the same runtime assumptions.

### Publishing-quality engineering lesson

For a book or course, the key teaching point is that packaging and runtime design are part of the platform architecture, not a thin afterthought bolted on at the end. A pipeline that cannot be packaged, promoted, and executed predictably is not complete, no matter how elegant its SQL may be.
...
Advanced practitioners should not lump all metadata together. Configuration metadata, execution metadata, and semantic metadata answer different questions.
...
The run-summary NDJSON pattern effectively gives the platform a memory of itself. That is a strong design for small teams. Instead of relying solely on ephemeral logs, the platform accumulates a structured history of run behavior in the same data estate where the business data lives.

This is more than convenience. It turns operations into an analyzable domain.
...
Taken together, these decisions form the real thesis of this book: a modern data platform is less about assembling fashionable tools than about choosing durable boundaries between storage, execution, semantics, and operations.

## Chapter 40: The Data Engineering Dashboard

The platform generates not only business data, but also operational data about its own execution. This is where the repository starts to become a genuinely teachable platform rather than a sequence of batch tasks.
...
Too many teams treat observability as an external concern to be solved later by an infrastructure team. In a data platform, that is a mistake. The pipeline’s own execution history is analytical data. It deserves a modeled surface just as much as business facts do.
...
Students often learn to model only business processes. A more mature perspective is to also model platform behavior. That is the difference between merely building pipelines and operating a data platform as an engineered system.

---

## Conclusion

The central claim of this handbook is simple: a modern data platform does not need to begin with heavyweight infrastructure to be real.
...
This repository proves that claim in code.

It also demonstrates a second claim that is less often stated: good data-platform engineering is mostly about boundaries. Know what to generate. Know what to hand-write. Know what belongs in configuration. Know what belongs in metadata. Know what belongs in the serving contract. Once those boundaries are well chosen, the implementation becomes clearer, more testable, and easier to evolve.

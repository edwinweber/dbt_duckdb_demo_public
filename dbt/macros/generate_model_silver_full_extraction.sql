...
{%- else %}
,CAST(MIN({{ parse_filename_ts('LKHS_filename') }}) OVER (PARTITION BY {{ primary_key_columns }}) AS DATETIME) AS LKHS_date_inserted_src
{%- endif %}
...
,CTE_FILES AS   (
                ...
                ,      LEAD({{ parse_filename_ts('LKHS_filename') }}) OVER (ORDER BY LKHS_filename) AS LKHS_date_valid_from_next
